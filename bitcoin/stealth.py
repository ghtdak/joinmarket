...
from bitcoin.main import multiply, sha256, encode_pubkey, add_pubkeys, privtopub, \
    add_privkeys, hex_to_b58check, b58check_to_hex, privkey_to_pubkey, \
    pubkey_to_address
...
def shared_secret_sender(scan_pubkey, ephem_privkey):
    ...
    shared_secret = sha256(encode_pubkey(shared_point,
                                                   'bin_compressed'))
...
def pubkeys_to_basic_stealth_address(scan_pubkey, spend_pubkey, magic_byte=42):
    ...
    hex_data = '00{0:066x}01{1:066x}0100'.format(int(hex_scankey, 16), int(
        hex_spendkey, 16))
...
def mk_stealth_metadata_script(ephem_pubkey, nonce):
    ...
    return op_return + msg_size + version + '{0:08x}'.format(
        nonce) + encode_pubkey(ephem_pubkey, 'hex_compressed')
...
def mk_stealth_tx_outputs(stealth_addr,
    ...
    pay_pubkey = uncover_pay_pubkey_sender(scan_pubkey, spend_pubkey,
                                           ephem_privkey)
    pay_addr = pubkey_to_address(pay_pubkey, magic_byte_addr)
