...
config_location = os.path.join(os.path.dirname(os.path.realpath(__file__)),
                               'joinmarket.cfg')
required_options = {
    'BLOCKCHAIN': ['blockchain_source', 'network', 'bitcoin_cli_cmd'],
...
class Wallet(object):
    ...
    def get_balance_by_mixdepth(self):
        ...
        for mixdepth, utxos in self.get_utxos_by_mixdepth().iteritems():
            mix_balance[mixdepth] = sum([addrval['value'] for addrval in
                                         utxos.values()])
        return mix_balance
    ...
    def select_utxos(self, mixdepth, amount):
        ...
        return dict([(i[
            ...
            'address': self.unspent[i['utxo']]['address']}) for i in inputs])
...
def choose_order(db, cj_amount, n):
    ...
    orders = [(o['counterparty'], o['oid'], calc_cj_fee(
        o['ordertype'], o['cjfee'], cj_amount)) for o in sqlorders
