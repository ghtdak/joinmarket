...
def rand_pow_array(power, n):
    ...
    return [y ** (1.0 / power)
            for y in [x * 0.0001 for x in random.sample(xrange(10000), n)]]
...
class AbstractWallet(object):
    ...
    def select_utxos(self, mixdepth, amount):
        ...
        return dict(
            [(i['utxo'],
              {'value': i['value'],
               'address': utxo_list[i['utxo']]['address']}) for i in inputs])
...
def choose_orders(db, cj_amount, n, chooseOrdersBy, ignored_makers=[]):
    ...
    orders = [(o['counterparty'], o['oid'], calc_cj_fee(
        o['ordertype'], o['cjfee'], cj_amount), o['txfee']) for o in sqlorders
              if cj_amount >= o['minsize'] and cj_amount <= o['maxsize'] and
...
def choose_sweep_orders(db,
    ...
    available_orders = sorted(available_orders,
                              key=lambda k: k[1])  #sort from smallest to biggest cj fee
    chosen_orders = []
