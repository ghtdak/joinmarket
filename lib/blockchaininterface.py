            ...
            def run(self):
                ...
                while not unconfirmed_txid:
                    ...
                    for unspent_list in data:
                        txs = set([str(txdata['tx']) for txdata in
                                   unspent_list['unspent']])
                        if not shared_txid:
                            shared_txid = txs
                        else:
                  ...
                  ) for sv in btc.deserialize(txinfo['tx']['hex'])['outs']])
...
class BitcoinCoreInterface(BlockchainInterface):
    ...
    def sync_addresses(self, wallet, gaplimit=6):
        ...
        wallet_name = 'joinmarket-wallet-' + btc.dbl_sha256(wallet.keys[0][0
                                                                    ])[:6]
        addr_req_count = 20
        wallet_addr_list = []
        for mix_depth in range(wallet.max_mix_depth):
