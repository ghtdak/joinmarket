...
class IRCMessageChannel(MessageChannel):
    ...
    def send_ioauth(self, nick, utxo_list, input_pubkey, cj_addr, change_addr,
                    sig):
        authmsg = (str(','.join(utxo_list)) + ' ' + input_pubkey + ' ' + cj_addr
                   + ' ' + change_addr + ' ' + sig)
        ...
        for command in message[1:].split(COMMAND_PREFIX):
            ...
            try:
                ...
                #taker commands
                    ...
                    btc_sig = chunks[5]
                    if self.on_ioauth:
                        self.on_ioauth(nick, utxo_list, input_pubkey, cj_addr,
                                       change_addr, btc_sig)
                elif chunks[0] == 'sig':
                    sig = chunks[1]
                    if self.on_sig:
    ...
    def __on_pubmsg(self, nick, message):
        ...
        for command in message[1:].split(COMMAND_PREFIX):
            ...
            if self.check_for_orders(nick, chunks):
                ...
                try:
                    if chunks[1][0] != 'v' or int(chunks[1][
                            1]) < MIN_PROTOCOL_VERSION:
