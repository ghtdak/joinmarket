...
def der_encode_sig(v, r, s):
    ...
    return safe_hexlify(b'\x30' + encode(len(left + right), 256, 1) + left +
                        right)
...
def txhash(tx, hashcode=None):
    ...
    if hashcode:
        return dbl_sha256(from_string_to_bytes(tx) + encode(int(hashcode), 256,
                                                            4)[::-1])
    else:
...
def apply_multisignatures(*args):
    ...
    if isinstance(tx, str) and re.match('^[0-9a-fA-F]*$', tx):
        return safe_hexlify(apply_multisignatures(binascii.unhexlify(tx), i,
                                                  script, sigs))
