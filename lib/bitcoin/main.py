...
def add_pubkeys(p1, p2):
    ...
    return encode_pubkey(base10_add(decode_pubkey(p1, f1), decode_pubkey(
        p2, f2)), f1)
...
def multiply(pubkey, privkey):
    ...
    if not isinf(pubkey) and (pubkey[0] ** 3 + B - pubkey[1] * pubkey[1]
                           ) % P != 0:
...
def privkey_to_pubkey(privkey):
    ...
    if f in ['bin', 'bin_compressed', 'hex', 'hex_compressed', 'decimal']:
        ...
        return encode_pubkey(base10_multiply(G, privkey), f.replace('wif',
                                                                    'hex'))
...
def subtract_pubkeys(p1, p2):
    ...
    return encode_pubkey(base10_add(decode_pubkey(p1, f1),
                                    (k2[0], (P - k2[1]) % P)), f1)
...
def ecdsa_raw_verify(msghash, vrs, pub):
    ...
    u1, u2 = z * w % N, r * w % N
    x, y = base10_add(base10_multiply(G, u1), base10_multiply(
...
def ecdsa_raw_recover(msghash, vrs):
    ...
    Qr = base10_add(neg_pubkey(base10_multiply(G, z)), base10_multiply(
        (x, y), s))
...
def ecdsa_recover(msg, sig):
    return encode_pubkey(ecdsa_raw_recover(electrum_sig_hash(msg),
                                           decode_sig(sig)), 'hex')
