...
def build_objects(argv=None):
    ...
    description = ('Does useful little tasks involving your bip32 wallet. The '
                   'method is one of the following: display - shows addresses '
                   'and balances. displayall - shows ALL addresses and '
                   'balances. summary - shows a summary of mixing depth '
                   'balances.generate - generates a new wallet recover - '
                   'recovers a wallet from the 12 word recovery seed showseed '
                   '- shows the wallet recovery seed and hex seed. '
                   'importprivkey - adds privkeys to this wallet (privkeys '
                   'are spaces or commas separated) listwallets - lists all '
                   'wallets with creator and timestamp')
    ...
    if method == 'display' or method == 'displayall' or method == 'summary':
        ...
        print('WARNING: This imported key will not be recoverable with your '
              '12 word mnemonic seed. Make sure you have backups. WARNING: '
              'Handling of raw ECDSA bitcoin private keys can lead to '
              'non-intuitive behaviour and loss of funds. Recommended instead '
        ...
        for privkey in privkeys:
            ...
            encrypted_privkey = jm.encryptData(wallet.password_key, privkey_bin)
