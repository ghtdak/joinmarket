...
class YieldGenerator(Maker):
    ...
    def create_my_orders(self):
        ...
        if len([b for m, b in mix_balance.iteritems()
                if b > common.DUST_THRESHOLD]) == 0:
        ...
        for cj in cjfee:
            ...
            if mix_balance[
                    max_mix] - common.DUST_THRESHOLD > 0 and minsize < mix_balance[
                        max_mix] - common.DUST_THRESHOLD:
                order = {'oid': oid,
                         'ordertype': 'relorder',
                         'minsize': minsize,
                         'maxsize':
                         mix_balance[max_mix] - common.DUST_THRESHOLD,
                         'txfee': txfee[oid],
                         'cjfee': cj}
                orders.append(order)
                oid += 1
    ...
    def oid_to_order(self, cjorder, oid, amount):
        ...
        mix_balance = {
            key: value
            for key, value in self.wallet.get_balance_by_mixdepth().items()
            if value - common.DUST_THRESHOLD <= filter(
                lambda orders: orders['oid'] == oid, orders)[0]['maxsize']
        }
        max_mix = max(mix_balance, key=mix_balance.get)
    ...
    def on_tx_unconfirmed(self, cjorder, txid, removed_utxos):
        ...
        if len(oldorders) != len(neworders):
            ...
            annOrders = [i
                         for i, j in zip(
                             sorted(neworders), sorted(oldorders)) if i != j]
