...
class YieldGenerator(Maker):
    ...
    def create_my_orders(self):
        ...
        filtered_mix_balance = [f for f in filtered_mix_balance
                                if f[1] > minsize]
        ...
        order = {'oid': 0,
                 ...
                 'maxsize': minsize - common.DUST_THRESHOLD,
                 'txfee': txfee,
                 'cjfee': absorder_fee}
        orders = [order] + orders
        return orders
    ...
    def on_tx_unconfirmed(self, cjorder, txid, removed_utxos):
        ...
        old_new_diff = [i
                        for i, j in zip(
                            sorted(oldorders), sorted(neworders)) if i != j]
        ...
        if len(neworders) == len(oldorders):
            ...
            cancel_orders = [o['oid']
                             for o in old_new_diff if o['oid'] not in ann_oids]
